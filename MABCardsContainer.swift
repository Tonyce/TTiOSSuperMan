...
class MABCardsContainer: UIView, UICollisionBehaviorDelegate, UIDynamicAnimatorDelegate {
    ...
    func discardAllSwipeableViews() {
        if let anchorViewAttachmentBehavior = self.anchorViewAttachmentBehavior {
            self.animator.removeBehavior(anchorViewAttachmentBehavior)
        }
    ...
    func animateSwipeableViewsIfNeeded() {
        ...
        if (self.isRotationEnabled) {
            ...
            if (numSwipeableViews >= 1) {
                if let swipeableViewSnapBehavior = self.swipeableViewSnapBehavior {
                    self.animator.removeBehavior(swipeableViewSnapBehavior)
                }
    ...
    func collisionBehavior(behavior: UICollisionBehavior, endedContactForItem item: UIDynamicItem, withBoundaryIdentifier identifier: NSCopying?) {
        ...
        for aBehavior in self.animator.behaviors {
            if (aBehavior.isKindOfClass(UIAttachmentBehavior)) {
                ...
                if let attachmentBehavior = aBehavior as? UIAttachmentBehavior {
                    let items = attachmentBehavior.items as NSArray
                    
                    if (items.containsObject(item)) {
                        self.animator.removeBehavior(attachmentBehavior as UIAttachmentBehavior)
            ...
            if (aBehavior.isKindOfClass(UIPushBehavior)) {
                if let attachmentBehavior = aBehavior as? UIPushBehavior {
                    let items = attachmentBehavior.items as NSArray
                    if (items.containsObject(item)) {
                        self.animator.removeBehavior(attachmentBehavior as UIPushBehavior)
                        viewsToRemove.addObjectsFromArray(items as [AnyObject])
                    }
                }
            }
            if (aBehavior.isKindOfClass(UICollisionBehavior)) {
                if let attachmentBehavior = aBehavior as? UICollisionBehavior {
                    let items = attachmentBehavior.items as NSArray
                    if (items.containsObject(item)) {
                        self.animator.removeBehavior(attachmentBehavior as UICollisionBehavior)
                        viewsToRemove.addObjectsFromArray(items as [AnyObject])
                    }
                }
            }
